...
- **C3 节日/主题图集**：当天命中节日（春节/中秋/国庆等在线国假 + 圣诞/情人节/七夕日期兜底）时，签到背景自动把节日推荐标签作为首选尝试，无结果自然回退用户配置标签
- **B2 签到速度榜播报**：群内首次签到即播报 🥇/🥈/🥉 今日签到前 3 名（复用 `today` 排行按 `first_seen_at` 排序，仅首次签到触发，重复签到不刷屏）
- **E3 群签到统计报表**：WebUI 运营视图新增「下载周报 CSV」（近 7/30 天），导出各群今日/本月签到、日均活跃、近 N 天签到人次、赛季榜第一（utf-8-sig 兼容 Excel），文件落在 `data/checkin_reports/`

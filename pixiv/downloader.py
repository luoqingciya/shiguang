    ...
    for index, raw_value in enumerate(values, 1):
        ...
        if len(origins) >= MAX_PROXY_ORIGINS:
            if not limit_warning_logged:
                logger.warning(
                    f"{LOG_PREFIX} Lolicon 图片反代地址超过上限: max_origins={MAX_PROXY_ORIGINS}"
                )
...
def rewrite_image_url_with_origin(url: str, origin: str) -> str:
    ...
    return urlunsplit((proxy.scheme, proxy.netloc, image.path, image.query, image.fragment))
...
class ImageDownloader:
        ...
        try:
            ...
            if fd >= 0:
                try:
                    ...
                    logger.warning(
                        f"{LOG_PREFIX} 关闭图片临时文件描述符失败: error_type={type(exc).__name__}"
                    )
            cleanup(path)
            raise
    ...
    async def download_for_send(
        ...
        def _plan_download_attempts(
            ...
            selected_proxy_urls = proxy_urls[: max(0, remaining_budget - reserved_for_returned)]
        ...
        for actual_quality, original_url in candidates:
            ...
            for route, url in download_routes:
                ...
                within_downgrade_limit = (
                    downgrade_limit_bytes <= 0 or file_size <= downgrade_limit_bytes
                )
                if (
                    actual_quality != "original" and not enforce_downgrade_limit
                ) or within_downgrade_limit:
                    quality_label = QUALITY_LOG_LABELS.get(actual_quality, actual_quality)
                    route_label = _download_route_log_label(route, illust.get("_source"))
                    logger.info(
            ...
            if quality_error is not None:
                logger.debug(
                    f"{LOG_PREFIX} {log_context} quality={actual_quality} 不可用，尝试下一画质"
                )
        ...
        if last_error is not None:
            ...
            message = f"图片下载失败: attempts={attempts} reason={last_reason}"
            if failure_reasons and all(reason == "timeout" for reason in failure_reasons):
...
def iter_image_quality_urls(illust: dict, quality: str = "original") -> Iterable[tuple[str, str]]:
...
def cleanup(path: str):
    try:
        ...
        logger.warning(f"{LOG_PREFIX} 清理图片临时文件失败: error_type={type(exc).__name__}")

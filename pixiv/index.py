...
class ImageIndexStore:
    ...
    async def set_retention_days(self, retention_days: int, *, cleanup: bool = True) -> int:
        ...
        async with self._lock:
            ...
            try:
                if cleanup:
                    removed = await asyncio.to_thread(self._cleanup_old_days_sync, date_key)
    ...
    def close(self) -> None:
        ...
        if self._conn is not None:
            try:
                ...
                logger.warning(f"{LOG_PREFIX} 关闭图片索引连接失败: 错误类型={type(exc).__name__}")
            self._conn = None
    ...
    async def get_page_offset(self, scope: str, source_key: str) -> int:
        ...
        async with self._lock:
            return await asyncio.to_thread(self._get_page_offset_sync, scope, source_key)

    async def advance_page_offset(self, scope: str, source_key: str, results_count: int) -> int:
    ...
    async def get_blacklist_thumbnail_path(self, illust_id: str) -> Path | None:
        ...
        if not illust_id:
            return None
        async with self._lock:
            return await asyncio.to_thread(self._get_blacklist_thumbnail_path_sync, illust_id)

    async def get_blacklist_thumbnail_paths(self, illust_ids: Iterable[str]) -> dict[str, Path]:
        ids = self._normalize_ids(illust_ids)
        if not ids:
            return {}
        async with self._lock:
            return await asyncio.to_thread(self._get_blacklist_thumbnail_paths_sync, ids)
    ...
    async def remove_blacklist_illust(self, illust_id: str) -> dict[str, Any] | None:
        ...
        if not illust_id:
            return None
        async with self._lock:
            return await asyncio.to_thread(self._remove_blacklist_illust_sync, illust_id)
    ...
    def _cleanup_old_days_sync(self, date_key: str) -> int:
        ...
        try:
            if self._retention_days == 0:
                ...
                cursor = conn.execute("DELETE FROM image_usage WHERE date_key < ?", (cutoff_key,))
            conn.commit()
        except Exception:
            conn.rollback()
            raise
    ...
    def _advance_page_offset_sync(self, scope: str, source_key: str, results_count: int) -> int:
        now = self.now_iso()
        conn = self._get_conn()
        row = conn.execute(
    ...
    def _get_used_illust_ids_sync(self, date_key: str, scope: str, source_key: str) -> set[str]:
        if self._retention_days == 0:
            return set()
        cutoff_key = self._retention_cutoff_key(date_key)
        conn = self._get_conn()
    ...
    def _get_custom_safety_terms_sync(self) -> set[str]:
        rows = (
            self._get_conn().execute("SELECT normalized_term FROM content_safety_terms").fetchall()
        )
    ...
    def _get_blacklist_thumbnail_paths_sync(self, illust_ids: list[str]) -> dict[str, Path]:
    ...
    def _safe_unlink(path: Path) -> None:
        try:
            ...
            logger.warning(f"{LOG_PREFIX} 清理黑名单缩略图失败: error_type={type(exc).__name__}")

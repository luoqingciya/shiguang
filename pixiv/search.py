...
from .._event import EventAdapter, Image, Node, Nodes, Plain
...
class SearchMixin:
    ...
    @staticmethod
    def _should_use_forward(*, downloaded_count: int, threshold: int, platform_name: str) -> bool:
        return platform_name == AIOCQHTTP_PLATFORM and downloaded_count > threshold

    def _ensure_client_or_error(self, event: EventAdapter) -> bool:
        ...
        self._init_client()
        return bool(
            (getattr(self, "lolicon_client", None) and self.lolicon_client.available)
            or getattr(self, "client", None)
        )

    @staticmethod
    def _event_scope(event: EventAdapter) -> str:
        ...
        *,
        ...
        if lolicon_client and lolicon_client.available:
            try:
                if tag:
                    ...
                    illusts = await lolicon_client.random(count=count, aspect_ratio=aspect_ratio)
        ...
        if use_page_cursor and page_offset == 0 and self.image_index is not None:
            try:
                ...
                logger.warning(
                    f"{LOG_PREFIX} 读取 Pixiv 回退分页游标失败: error_type={type(exc).__name__}"
                )
    ...
    async def _handle_search(
        ...
        if wait > 0:
            logger.debug(f"{LOG_PREFIX} 搜索请求触发频率限制: retry_after_seconds={wait}")
        ...
        try:
            ...
            return
        if not illusts:
            yield event.plain_result("😶 可用作品都被内容安全策略过滤了，换个标签后再试")
            return

        pick_count = min(count, len(illusts))
        dedupe_days = self._cfg_int("dedupe_days", 1, 0, 7)
        if self.image_index is not None and self.image_index.retention_days != dedupe_days:
            try:
        ...
        if not chosen:
            yield event.plain_result("当前去重范围内没有未发送过的图片了，换个标签或稍后再试")
            return
        ...
        try:
            ...
            if use_forward:
                ...
                if not forward_success:
                    ...
                    for illust, _path, _actual_q, _file_size in downloaded:
                        await self._record_image_usage(
                            event,
                            source_key,
                            illust,

...
class PixivClient:
    ...
    async def _ensure_logged_in_locked(self) -> None:
        ...
        try:
            ...
            raise RuntimeError("未安装 pixivpy-async，请运行: pip install pixivpy-async") from None
    ...
    async def _wait_for_with_retry(self, factory, *, operation: str):
        ...
        try:
            ...
            if self._closed:
                raise
            logger.warning(
                f"{LOG_PREFIX} Pixiv {operation}失败，1 秒后重试: error_type={type(exc).__name__}"
            )
    ...
    async def close(self):
        async with self._lock:
            self._closed = True
            try:
                await asyncio.wait_for(self._wait_until_idle_locked(), timeout=self.close_timeout)
            except asyncio.TimeoutError:
                logger.warning(
                    f"{LOG_PREFIX} Pixiv 客户端关闭等待超时: active_calls={self._active_calls}"
                )
            api = self._api
            self._api = None
        await self._close_api(api)
    ...
    async def _close_api(self, api) -> None:
        ...
        if callable(close):
            try:
                ...
                logger.warning(
                    f"{LOG_PREFIX} 关闭 Pixiv API 会话失败: error_type={type(exc).__name__}"
                )

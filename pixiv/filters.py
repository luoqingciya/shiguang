...
class FiltersMixin:
    ...
    async def _blacklist_reason_for_illust(self, illust: dict, illust_id: str = "") -> str:
    ...
    async def _pick_illusts(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"{LOG_PREFIX} 读取发图去重索引失败: error_type={type(e).__name__}")
            return []
        ...
        if not fresh and raw_count > 0:
            try:
                ...
                logger.warning(f"{LOG_PREFIX} 分页游标更新失败: error_type={type(e).__name__}")

        candidates = random.sample(fresh, len(fresh)) + random.sample(repeated, len(repeated))
        chosen: list[dict] = []

...
class CheckinArtworkMixin:
        ...
        try:
            ...
            if source.startswith("lolicon:"):
                ...
                if not illust:
                    ...
                    return replace(saved, mode="fallback")
            if await self._blacklist_reason_for_illust(illust, record.background_illust_id):
                logger.warning(
        ...
        *,
        ...
        for page_attempt in range(1, CHECKIN_BACKGROUND_PAGE_ATTEMPTS + 1):
            ...
            if source_key != used_source_key:
                ...
                used_ids.update(await self._checkin_background_used_ids(event, source_key))
        ...
        for idx, illust in enumerate(ordered[:8], 1):
            ...
            claimed = False
            if claim_usage:
                claimed = await self._claim_checkin_background_usage(event, source_key, illust_id)
                if not claimed:
                    logger.debug(
            ...
            try:
                ...
                if claimed and not background_ready:
                    await self._release_checkin_background_usage(event, source_key, illust_id)
        return None
    ...
    async def _claim_checkin_background_usage(
        self, event: EventAdapter, source_key: str, illust_id: str
    ) -> bool:
    ...
    async def _release_checkin_background_claim(
        ...
        if (
            ...
            return
        await self._release_checkin_background_usage(event, background.source, background.illust_id)

    async def _checkin_background_used_ids(self, event: EventAdapter, source_key: str) -> set[str]:
        used_ids: set[str] = set()
        if self.image_index is not None:
            try:
                used_ids.update(
                    await self.image_index.get_used_illust_ids(self._event_scope(event), source_key)
                )
            except Exception as e:
                logger.warning(
                    f"{LOG_PREFIX} 签到背景读取去重索引失败: error_type={type(e).__name__}"
                )
        return used_ids

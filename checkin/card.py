...
from .content import MILESTONES, CheckinContent
...
def get_checkin_card_template(theme_id: str = DEFAULT_CHECKIN_THEME_ID) -> str:
    ...
    if _CSS_MARKER not in html:
        raise RuntimeError(f"check-in card theme {theme.theme_id} is missing its CSS marker")
    if _FONT_DATA_MARKER not in css:
        raise RuntimeError(f"check-in card theme {theme.theme_id} is missing its font marker")
...
def build_checkin_card_data(
    *,
    ...
    if (width, height) != (CHECKIN_CARD_WIDTH, CHECKIN_CARD_HEIGHT):
        raise ValueError(f"check-in card canvas must be {CHECKIN_CARD_WIDTH}x{CHECKIN_CARD_HEIGHT}")
    ...
    data.update(
        {
            ...
            "affection_progress": int(affection_level(view_model.affection_value)["progress"]),
...
def _artwork_credit(view_model: CheckinCardViewModel) -> str:
    if not any((view_model.artwork_title, view_model.artwork_author, view_model.artwork_id)):
...
def _encode_image_data_url(file_path: Path) -> str:
    try:
        ...
        logger.warning(f"{LOG_PREFIX} 签到背景图片处理失败: error_type={type(e).__name__}")
        return ""

...
class FeatureStoreMixin:
    ...
    async def get_user_preference(self, user_id: str) -> CheckinUserPreference:
        async with self._lock:
            return await asyncio.to_thread(self._get_or_create_preference_sync, str(user_id or ""))
    ...
    async def clear_birthday(self, user_id: str) -> CheckinUserPreference:
        async with self._lock:
            return await asyncio.to_thread(self._clear_birthday_sync, str(user_id or ""))

    async def mark_qq_birthday_checked(self, user_id: str) -> None:
        async with self._lock:
            await asyncio.to_thread(self._mark_qq_birthday_checked_sync, str(user_id or ""))
    ...
    async def list_achievements(self, user_id: str) -> tuple[str, ...]:
        async with self._lock:
            return await asyncio.to_thread(self._list_achievements_sync, str(user_id or ""))
    ...
    async def list_owned_theme_ids(self, user_id: str) -> tuple[str, ...]:
        async with self._lock:
            return await asyncio.to_thread(self._list_owned_theme_ids_sync, str(user_id or ""))
    ...
    async def add_global_event(
        ...
        normalized_type, normalized_date = _validate_global_event_date(event_type, date_value)
    ...
    def _purchase_theme_sync(
        ...
        with closing(self._connect()) as conn:
            ...
            try:
                ...
                cost = int(theme_row["price"] or 0) if configured_cost is None else configured_cost
    ...
    def _select_theme_sync(self, user_id: str, theme_id: str) -> str:
        ...
        with closing(self._connect()) as conn:
            ...
            template_version = f"{theme_id}:{int(theme_row['version'] or 1)}"
            conn.execute(
                "UPDATE checkin_users SET current_theme_id = ?, updated_at = ? WHERE user_id = ?",
                (theme_id, now, user_id),
            )
            conn.execute(
                """

...
def validate_checkin_snapshot(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    events = [
        _normalize_global_event_row(row, i) for i, row in enumerate(collections["global_events"])
    ]
    achievements = [
        _normalize_achievement_row(row, i) for i, row in enumerate(collections["achievements"])
    ]
    user_themes = [
        _normalize_user_theme_row(row, i) for i, row in enumerate(collections["user_themes"])
    ]
    group_presence = [
        _normalize_group_presence_row(row, i) for i, row in enumerate(collections["group_presence"])
    ]
...
def _normalize_user_row(row: Any, index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "last_checkin_date": _require_text(row, "last_checkin_date", location, allow_blank=True),
        "boost_start_date": _require_text(row, "boost_start_date", location, allow_blank=True),
        "boost_until_date": _require_text(row, "boost_until_date", location, allow_blank=True),
        ...
        "qq_birthday_checked": _require_boolish_int(row, "qq_birthday_checked", location),
...
def _normalize_record_row(
    ...
    *,
    ...
    return {
        ...
        "background_mode": _require_text(row, "background_mode", location, allow_blank=True),
        "background_source": _require_text(row, "background_source", location, allow_blank=True),
        "background_illust_id": _require_text(
            row, "background_illust_id", location, allow_blank=True
        ),
        "background_title": _require_text(row, "background_title", location, allow_blank=True),
        "background_author": _require_text(row, "background_author", location, allow_blank=True),

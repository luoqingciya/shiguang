...
class RankingStoreMixin:
    ...
    def _get_group_ranking_sync(
        ...
        for user_id, user_rows in grouped.items():
            ...
            if ranking_type == "today":
                ...
                matching_dates = [item for item in dates if item.startswith(month + "-")]
                if not matching_dates:
                    continue
                value = len(matching_dates)
                month_rows = [
                    item for item in user_rows if str(item["date_key"]).startswith(month + "-")
                ]
    ...
    def _get_group_trend_sync(self, group_id: str, days: int) -> list[dict[str, object]]:

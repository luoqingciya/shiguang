...
from shiguang.pixiv.constants import AIOCQHTTP_PLATFORM
from shiguang.pixiv.lolicon import LoliconClient
from shiguang.pixiv.search import SearchMixin
...
class LoliconClientTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_request_and_normalization_keep_all_sizes(self):
        ...
        self.assertEqual(
            [value for key, value in params if key == "size"],
    ...
    async def test_tagged_lolicon_success_does_not_call_pixiv(self):
        ...
        illusts, _, source = await plugin._fetch_source_candidates(_SourceEvent(), "初音ミク")
    ...
    async def test_untagged_lolicon_failure_uses_pixiv_recommended(self):
        ...
        illusts, _, source = await plugin._fetch_source_candidates(_SourceEvent(), "")

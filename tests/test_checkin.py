...
class CheckinStoreTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_repeated_duplicate_checkins_never_change_affection(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenCheckinStore(tmp)
            first = await store.checkin(user_id="10001", username="tester", bot_name="neko")
    ...
    async def test_record_content_persists_once_and_only_local_can_upgrade_to_ai(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenCheckinStore(tmp)
            checked = await store.checkin(user_id="10001", username="tester", bot_name="neko")
    ...
    async def test_competing_ai_upgrades_write_once_and_return_winning_record(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            await first_store.checkin(user_id="10001", username="tester", bot_name="neko")
            ...
            with closing(sqlite3.connect(Path(tmp) / "checkin.sqlite3")) as conn:
                conn.execute("CREATE TABLE content_update_audit (greeting TEXT NOT NULL)")
                conn.execute(
                    """
            ...
            with closing(sqlite3.connect(Path(tmp) / "checkin.sqlite3")) as conn:
                audit_count = conn.execute("SELECT COUNT(*) FROM content_update_audit").fetchone()[
                    0
                ]
    ...
    async def test_streak_continues_by_beijing_date_and_resets_after_gap(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            day1 = await store.checkin(user_id="10001", username="tester", bot_name="neko")
            store.date_key = "2026-05-27"
            day2 = await store.checkin(user_id="10001", username="tester", bot_name="neko")
            store.date_key = "2026-05-29"
            day4 = await store.checkin(user_id="10001", username="tester", bot_name="neko")
    ...
    async def test_snapshot_rejects_invalid_greeting_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "greeting_source"):
                load_checkin_snapshot_json(dump_checkin_snapshot_json(snapshot).encode("utf-8"))

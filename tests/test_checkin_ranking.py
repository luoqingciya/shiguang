...
async def test_group_rankings_and_trend_use_only_current_group() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        await store.checkin(user_id="10002", username="Bob", bot_name="neko", group_id="20002")
        store.date_key = "2026-07-13"
        await store.checkin(user_id="10001", username="Alice", bot_name="neko", group_id="20001")
        await store.checkin(user_id="10002", username="Bob", bot_name="neko", group_id="20001")

        month = await store.get_group_ranking(group_id="20001", ranking_type="month", limit=10)
        streak = await store.get_group_ranking(group_id="20001", ranking_type="streak", limit=10)

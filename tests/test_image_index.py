...
from shiguang.pixiv.index import ImageIndexStore
...
class ImageIndexStoreTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_blacklist_list_order_and_remove(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual([record["illust_id"] for record in records], ["302", "301"])
    ...
    async def test_custom_safety_terms_and_blacklist_reason_persist(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenImageIndexStore(tmp)
            try:
                self.assertTrue(await store.add_safety_term("危险主题", added_by="admin"))
                self.assertIn("危险主题".casefold(), await store.get_custom_safety_terms())
                await store.add_blacklist_illust(
                    illust_id="350",
                    reason="构图不适合",
                    added_by="web",
    ...
    async def test_page_offset_ttl_expired(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                from shiguang.pixiv.index import PAGE_CURSOR_TTL_DAYS
    ...
    async def test_page_offset_ttl_valid(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            store = FrozenImageIndexStore(tmp)
            try:
                await store.advance_page_offset("group:1", "search:blue_archive", results_count=30)

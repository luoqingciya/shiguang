...
from shiguang.checkin import CheckinProfile, CheckinRecord
from shiguang.checkin.content import resolve_checkin_content
from shiguang.checkin.holiday import OnlineHoliday
...
def test_event_priority_birthday_then_custom_then_holiday() -> None:
    holiday = make_record(date_key="2026-10-01", total_days_after=30, streak_days_after=7)
...
def test_event_priority_holiday_then_milestone_then_streak_then_normal() -> None:
    ...
    streak = resolve_checkin_content(make_record(total_days_after=31, streak_days_after=7), profile)
    normal = resolve_checkin_content(make_record(total_days_after=31, streak_days_after=6), profile)
...
def test_known_lunar_festivals_are_resolved_with_lunar_python() -> None:
    ...
    spring_festival = resolve_checkin_content(make_record(date_key="2026-02-17"), profile)
...
def test_event_stack_keeps_secondary_events_and_workday_note() -> None:
    record = make_record(date_key="2026-07-11", total_days_after=30, streak_days_after=7)

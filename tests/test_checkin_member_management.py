...
async def test_member_search_update_and_history_isolation() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        by_name = await store.list_checkin_members(query="Alice New", limit=50, offset=0)
        ...
        with closing(sqlite3.connect(store._db_path)) as conn:
            indexes = {row[1] for row in conn.execute("PRAGMA index_list(checkin_records)")}
            presence_indexes = {
                row[1] for row in conn.execute("PRAGMA index_list(checkin_group_presence)")
            }

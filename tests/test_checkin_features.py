...
async def test_birthday_manual_priority_clear_and_qq_checked_state() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        manual = await store.set_birthday(user_id="10001", month=7, day=11, source="manual")
...
async def test_qq_birthday_never_overwrites_manual_value() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = await store.set_qq_birthday_if_not_manual(user_id="10001", month=8, day=8)
